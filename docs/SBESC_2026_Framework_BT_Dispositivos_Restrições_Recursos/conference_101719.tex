...
\textit{Behavior Trees} (BTs) são um formalismo de controle que se tornou bastante popular em aplicações de robótica e IA nos últimos anos. Originalmente, \textit{Behavior Trees} foram projetadas para superar limitações encontradas no desenvolvimento de inteligência artificial para personagens de jogos digitais utilizando Máquinas de Estado Finitas (MEF), por permitirem maior modularidade e reatividade. Ao longo dos anos, no entanto, tem-se observado uma crescente utilização de BTs em diversas áreas, como a robótica, sistemas autônomos, IA e controle industrial. Apesar do uso extensivo em diversas áreas, a aplicação de BTs em Sistemas Embarcados com Recursos Restritos tem sido pouco investigada, e não se encontram trabalhos na literatura que explorem o uso desse formalismo diante das restrições específicas de aplicações dessa área.

Um dos motivos possíveis pelos quais BTs não têm sido exploradas na área de Sistemas Embarcados com Recursos Restritos é a falta de bibliotecas ou \textit{frameworks} que possam ser integrados ao ambiente de desenvolvimento destas aplicações. O trabalho \cite{iovino_survey_2022} apresenta uma lista de bibliotecas de BTs disponíveis até o momento do estudo implementadas em diferentes linguagens de programação como Python, Java, C\#, C++ e JavaScript. No entanto, ambientes de desenvolvimento para Sistemas Embarcados com Recursos Restritos normalmente têm suas aplicações desenvolvidas em C. Desta maneira, a utilização direta destas bibliotecas se torna impeditiva.

[A FAZER] Comentar também sobre a modelagem da árvore utilizando a abordagem \textit{Left-Child Right-Sibling} como uma maneira mais eficiente para implementar em Sistemas Embarcados com Recursos Restritos, uma vez que não é necessário que um nó da árvore conheça todos os seus filhos, limitando-se apenas a conhecer o seu filho mais à esquerda e o nó irmão na estrutura da árvore.

[Contextualizar a área de Sistemas Embarcados com Recursos Restritos]

Com o intuito de preencher essa lacuna, este trabalho propõe um \textit{framework} de \textit{Behavior Trees} para Sistemas Embarcados com Recursos Restritos, chamado \textbf{BTreeFy}. O desenvolvimento deste \textit{framework} tem por objetivo atender às demandas específicas das aplicações de Sistemas Embarcados Com Recursos Restritos. Desta maneira, as principais contribuições deste trabalho são: (i) a implementação de um \textit{framework} funcional que permite o desenvolvimento na linguagem C de aplicações de Sistemas Embarcados com Recursos Restritos usando \textit{Behavior Trees}; (ii) uma ferramenta de geração de código automático a partir do editor visual \textit{Groot 2}; (iii) uma representação de Behavior Tree utilizando a abordagem \textit{left-child right-sibling}; (iv) uma implementação iterativa.
